export function buildGameAnalysis(history, answer, range = { min: 1, max: 100, precision: 0 }) {
  ...
  return {
    ...
    maxDistance: range.precision === 0 ? Math.abs(range.max - range.min) + 1 : Math.abs(range.max - range.min),
