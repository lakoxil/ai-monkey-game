...
export function scoreMode2(guess, answer, range = DEFAULT_RANGE) {
  ...
  return {
    feedback: isCorrect ? `猜中了！Score = ${formatScore(100)}` : "已取得分數",
    score,
    isCorrect,
  };
}
...
function clampScore(score, isCorrect = false) {
  const maxScore = isCorrect ? 100 : 100 - 1 / 10 ** SCORE_PRECISION;
  const clamped = Math.max(0, Math.min(maxScore, score));
  return roundScore(clamped);
}

export function formatScore(score) {
  return Number.isFinite(score) ? score.toFixed(SCORE_PRECISION) : "";
}
